...
# Candidate paths for a system Chrome/Chromium to use as Playwright fallback
_CHROME_PATHS = [
...
def _fetch_html_via_playwright(url: str) -> str:
...
def fetch_song_data(url: str) -> dict:
    ...
    try:
        ...
        log.debug("requests failed (TLS?), trying Playwright fallback for %s", url)
    ...
    if html is None:
        try:
            html = _fetch_html_via_playwright(url)
